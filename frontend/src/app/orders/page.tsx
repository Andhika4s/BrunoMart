        ...
        <div className="text-center py-12 px-6 bg-white border border-slate-200 rounded-2xl shadow-sm max-w-md w-full space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-2 leading-relaxed">
              Terjadi kesalahan saat mengambil data pesanan dari server.
                ...
                <Card key={order.id} className="overflow-hidden shadow-sm border-slate-200 bg-white rounded-2xl">
                  <CardHeader className="p-4 bg-slate-50/50 border-b border-slate-100 flex flex-row items-center justify-between flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-3">
                      {/* 💡 TOMBOL CETAK NOTA DENGAN KONDISI DISABLED JIKA BELUM COMPLETED */}
                      <button
                        onClick={() => setSelectedOrder(order)}
                        disabled={!isCompleted}
                        title={!isCompleted ? "Nota baru bisa dicetak setelah pesanan di-acc admin" : "Cetak Nota"}
                        className="bg-white hover:bg-slate-50 text-slate-700 text-xs font-bold px-3 py-1.5 rounded-xl border border-slate-200 flex items-center gap-1.5 transition shadow-sm disabled:opacity-40 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
                      >
                        <Printer size={13} className={isCompleted ? "text-slate-500" : "text-slate-300"} /> Cetak Nota
                      </button>
